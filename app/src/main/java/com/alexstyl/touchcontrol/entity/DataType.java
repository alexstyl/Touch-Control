    ...
    public CharSequence getDisplayingLabel(Resources res) {
        if (label == null)
        {
            if (dataType == TYPE_PHONE) {
                label = ContactsContract.CommonDataKinds.Phone.getTypeLabel(res, type, label).toString();
            } else if (dataType == TYPE_EMAIL) {
                label = ContactsContract.CommonDataKinds.Email.getTypeLabel(res, type, label).toString();
            }
        }
        return label;
    }
